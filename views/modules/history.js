export async function render(mrn) {
 ...
 <div class="mx-auto max-w-7xl sm:px-6 lg:px-8 py-6">
  <div class="px-4 sm:px-0">
    ...
    <p class="mt-1 max-w-2xl text-sm/6 text-gray-500">Personal details and clinical history.</p>
  </div>
  ...
  <form id="note-form" class="bg-white p-6 rounded-2xl shadow-xl w-full max-w-md space-y-4">
    ...
    <textarea name="text" placeholder="Enter consult note..." required class="w-full border border-gray-300 rounded px-3 py-2 h-32"></textarea>
    <div class="flex justify-end gap-2 pt-4">
      <button type="button" id="close-note-modal" class="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300">Cancel</button>
      <button type="submit" class="px-4 py-2 bg-sky-900 text-white rounded hover:bg-sky-800">Add Note</button>
    </div>
  </form>
</div>
    <dl class="divide-y divide-gray-100">
      ...
      <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
        ...
        <dd class="mt-1 text-sm/6 text-gray-700 sm:col-span-2 sm:mt-0">${data.patient.address}</dd>
      </div>
      <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
        <dt class="text-sm/6 font-medium text-gray-900">Blood Type</dt>
        <dd class="mt-1 text-sm/6 text-gray-700 sm:col-span-2 sm:mt-0">${data.patient.blood_type}</dd>
      </div>
              ...
              <li class="w-full flex items-center justify-between py-4 pr-5 pl-4 text-sm/6">
                <div class="w-full divide-y divide-gray-200 overflow-hidden rounded-lg bg-white shadow-lg border">
                  <div class="w-full px-4 py-2 sm:px-6 font-medium text-gray-700 bg-sky-50 flex items-center justify-between">
                    <div>
                        <span>${note.time}</span><span class="font-semibold text-gray-400"> - Dr. ${note.doctor_name}</span>
                    </div>
...
export async function afterRender(mrn) {
  ...
  const openBtn     = document.querySelector('button.bg-sky-800'); // "Add consult note" button
  ...
  form.onsubmit = async e => {
    ...
    const postRes = await fetch('http://localhost/HospiCareDev/BACKEND/public/clinical/notes/add', {
      method: 'POST',
      headers: {
    ...
    if (postRes.ok) {
      alert('Consult note added.');
      modal.classList.add('hidden');
      location.reload(); // or trigger router() if available
    } else {
  ...
  btn.onclick = async (e) => {
    ...
    if (!confirm('Are you sure you want to delete this consult note?')) return;

    const res = await fetch(`http://localhost/HospiCareDev/BACKEND/public/clinical/notes/delete?id=${noteId}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${token}`,
      },
